...
try {
  const backendResult = spawnSync('npx', ['tsx', 'scripts/test-backend-pack.ts', '--example', example, '--habit', habit, '--input', input], {
    cwd: ROOT,
    stdio: 'inherit',
    encoding: 'utf-8',
  });
...
// Run Browser Pack Test
console.log('┌─────────────────────────────────────────────────────────────────┐');
console.log('│                     BROWSER PACK TEST                           │');
console.log('└─────────────────────────────────────────────────────────────────┘\n');

const browserStart = Date.now();
try {
  const browserResult = spawnSync('npx', ['tsx', 'scripts/test-browser-pack.ts', '--example', example, '--habit', habit, '--input', input], {
    cwd: ROOT,
    stdio: 'inherit',
    encoding: 'utf-8',
  });
  ...
  if (browserResult.status !== 0) {
    throw new Error(`Process exited with code ${browserResult.status}`);
  }

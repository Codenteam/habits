...
/**
 * Generate packages/core/src/modules.ts defaultModules from catalog bits.
 * Keeps implicit `habits base` setup / Base pre-install in sync with the bits shown in the UI.
 */
function generateDefaultModules(bits: BitData[]): void {
  const catalogBits = bits
    .filter((b) => b.level !== 'l0')
    .sort((a, b) => a.packageName.localeCompare(b.packageName));

  const modulesFilePath = join(rootDir, 'packages/core/src/modules.ts');
  const content = `/**
...
export const defaultModules: ModulesConfig = {
  modules: [
${catalogBits.map((b) => `    { framework: 'bits', source: 'npm', repository: '${b.packageName}' },`).join('\n')}
  ],
};
`;

  writeFileSync(modulesFilePath, content);
  console.log(`  📄 packages/core/src/modules.ts (${catalogBits.length} bits)`);
}

 ...
 *
 * Usage:
 *   pnpm nx dev @ha-bits/examples-runner
 *   # or directly:
 *   npx tsx packages/examples-runner/src/tui.tsx
 * =============================================================================
 */
...
class ProcessManager {
  ...
  runYamlTest(example: ExampleInfo, testFile: string): string {
    ...
    this.appendLine(proc, `$ npx nx bits-test -f ${testFile}`);
    this.appendLine(proc, '');

    const child = spawn('npx', ['nx', 'bits-test', '-f', testFile], {
      cwd: PROJECT_ROOT,
      stdio: ['ignore', 'pipe', 'pipe'],
    });

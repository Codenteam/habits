...
/// Decode audio bytes to PCM samples (for base64 input)
pub fn pcm_decode_bytes(audio_bytes: &[u8]) -> Result<(Vec<f32>, u32)> {
    use symphonia::core::audio::{AudioBufferRef, Signal};
    use symphonia::core::codecs::{DecoderOptions, CODEC_TYPE_NULL};
    use symphonia::core::conv::FromSample;
...
/// Convert PCM samples to WAV and return as base64 data URL
pub fn pcm_to_wav_base64(samples: &[f32], sample_rate: u32) -> Result<String> {
    let mut buffer = Vec::new();
    write_pcm_as_wav(&mut buffer, samples, sample_rate)
        .map_err(|e| LocalAiError::Audio(format!("Failed to encode WAV: {e}")))?;

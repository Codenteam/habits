...
- **Test via Tauri WebDriver MCP tools** (primary testing method): `packages/manage/forge/src/mcp/tauri-commands.ts` (exposed via `habits-mcp` → `mcp.ts`)
...
1. **MCP Server** ([packages/manage/forge/src/mcp/tauri-commands.ts](packages/manage/forge/src/mcp/tauri-commands.ts), wired through [mcp.ts](packages/manage/forge/src/mcp/mcp.ts)):

...
- When running Cortex server and sending requests in the same script/session, always use `nohup` with `&` to run the server in the background: nohup npx nx dev @ha-bits/cortex --config showcase/hello-world/stack.yaml &
...
- If you need to test a habit, test showcase/hello-world/stack.yaml or showcase/marketing-campaign/stack.yaml
...
cd /tmp && nohup npx /path/to/habits/dist/packages/habits cortex --config /path/to/habits/dist/packages/habits/showcase/hello-world/stack.yaml > /tmp/habits-cortex.log 2>&1 &
...
- If I ask you "test all habits scenarios locally using npx", you should:
  ...
  **Base Mode Test:**
  ...
  curl http://localhost:3000/habits/base/api/templates/hello-world/stack.yaml                    # Templates → 200
...
- If I ask you "test all habits scenarios published in npx", you should:
  ...
  3. For cortex, use example from the package: `~/.npm/_npx/*/node_modules/habits/showcase/hello-world/stack.yaml`

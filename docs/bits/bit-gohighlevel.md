...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      ...
      const stats = data.stats['@ha-bits/bit-gohighlevel']
      if (stats) {
        const el = document.querySelector('[data-package="@ha-bits/bit-gohighlevel"] .download-count')
...
<div class="bit-meta">
  ...
  <span class="bit-downloads" data-package="@ha-bits/bit-gohighlevel">📥 <span class="download-count">-</span> downloads</span>

---
title: "If / Branch"
description: "Conditional logic bit for branching workflows with if/else and switch statements"
aside: false
---
...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      ...
      const stats = data.stats['@ha-bits/bit-if']
      if (stats) {
        const el = document.querySelector('[data-package="@ha-bits/bit-if"] .download-count')
...
# <component :is="GitBranch" :size="32" class="inline-icon" /> If / Branch

<div class="bit-meta">
  <span class="bit-package">`@ha-bits/bit-if`</span>
  <span class="bit-version">v1.0.1</span>
  <span class="bit-downloads" data-package="@ha-bits/bit-if">📥 <span class="download-count">372</span> downloads</span>

...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      ...
      const stats = data.stats['@ha-bits/bit-snov']
      if (stats) {
        const el = document.querySelector('[data-package="@ha-bits/bit-snov"] .download-count')
...
<div class="bit-meta">
  ...
  <span class="bit-downloads" data-package="@ha-bits/bit-snov">📥 <span class="download-count">-</span> downloads</span>

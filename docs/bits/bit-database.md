...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      ...
      const stats = data.stats['@ha-bits/bit-database']
      if (stats) {
        const el = document.querySelector('[data-package="@ha-bits/bit-database"] .download-count')
...
<div class="bit-meta">
  ...
  <span class="bit-downloads" data-package="@ha-bits/bit-database">📥 <span class="download-count">247</span> downloads</span>

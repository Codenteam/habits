---
title: "Airtable"
description: "Create records, manage schema, and poll for new rows with @ha-bits/bit-airtable"
---

# Airtable

Use `@ha-bits/bit-airtable` to create and read records, auto-create table columns, and poll for new rows with a personal access token (PAT).

**Related bit:** `@ha-bits/bit-airtable`
...
- `data.records:write`
- `schema.bases:read`
- `schema.bases:write` — required to **create fields** via the [Create field API](https://airtable.com/developers/web/api/create-field)
...
You can use an Omni-generated base for testing, but **Step 3** below explains creating a dedicated base and clearing default fields for Habits workflows.

### 2. Create a personal access token (PAT)

1. Open **[airtable.com/create/tokens](https://airtable.com/create/tokens)**.
2. Click **Create token**.
3. Give it a name (e.g. `habits-airtable`).
4. Under **Scopes**, enable all of these:
   - `data.records:read`
   - `data.records:write`
   - `schema.bases:read`
   - `schema.bases:write` — required so workflows can **create columns** via the API
5. Under **Access**, add the **workspace** where you will create your base.
...
4. **Remove the default columns** so Habits can create its own from a workflow UI:
   - For each existing column header, open the column menu → **Delete field** (or **Customize fields** and remove them).
   - Leave the table with **no custom fields** (or only fields you do not plan to manage from Habits).
...
Add the values to your habit or showcase `.env`:
...
|------|------|-------------|
| Action | `createRecord` | Create a record in a table |
| Action | `listRecords` | List records from a table |
| Action | `getRecord` | Get a single record by ID |
| Action | `listTables` | List tables and field metadata |
| Action | `createField` | Create a column |
| Action | `ensureFields` | Create missing columns from names |
| Trigger | `newRecords` | Poll every minute; return new records (deduplicated via `pollingStore`) |

### Auto field types

When creating columns with `ensureFields`, types are inferred from the name:
...
### Polling trigger notes

The `newRecords` trigger runs on a cron schedule (default every 1 minute). Each poll uses the same API as `listRecords`, skips record IDs already in `pollingStore`, and returns only new rows.

For polling workflows, pass the PAT in trigger **params** (not only `credentials`):
...
<IntegrationShowcases integration="airtable" />
